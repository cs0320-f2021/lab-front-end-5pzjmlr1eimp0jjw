...
function addTwoNumbers () {
    let val1 = document.querySelector('#first-input').value
    let val2 = document.querySelector('#second-input').value
...
function displayCart () {
    ...
    const displayItem = function (item) {
       ...
       // for(let i = 0; items.length; i++) {
            ...
            cart.innerHTML += `<div id="${item.id}">Item ${item.id}, quantity ${quantity}, price ${parseInt(price) * quantity}</div>`
    ...
    const itemsArr = Object.keys(myCart)
    let totalPrice = 0;
    itemsArr.map(a => {
        let item = document.getElementById(a);
        displayItem(item);
        totalPrice += myCart[item.id] * parseInt(item.getAttribute("data-price"));
    })
